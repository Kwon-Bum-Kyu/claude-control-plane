...
[축 A] 사용자 명시 신호    (슬래시, --effort, --sandbox, --fallback-claude)
     ↓ (매칭 없으면)
[축 B] 입력 크기           (>30K → Antigravity, 5K~30K + review 키워드 → Codex)
     ↓ (매칭 없으면)
[축 C] 키워드 카탈로그     (codex > antigravity > claude; main-context-bind 는 claude 강제)
     ↓ (매칭 없으면)
...
| `--max-tokens` | Antigravity (Antigravity 전용 플래그) |
...
3. **Antigravity 키워드** (`summarize`, `extract`, `요약`, `로그` 등) → Antigravity.
...
router-suggest 훅은 `UserPromptSubmit` 시 동작합니다. 동일한 결정을 계산해, 결과가 `agy` 또는 `codex` 인 경우 `[CCP-ROUTER-001]` system reminder 로 적합한 슬래시 커맨드를 추천합니다. **자동 위임은 하지 않습니다.** 사용자가 직접 슬래시를 타이핑합니다. 원칙 4 (위임 실패 시 자동 fallback 금지) 와 정합합니다.
