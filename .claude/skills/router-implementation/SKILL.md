...
CCP의 라우터는 작업을 Claude 본체 처리 vs Antigravity 위임으로 분류한다. 합격 기준은 정확도 80%+ (R3 완화).
...
### 1. 사용자 명시 (최우선)
사용자가 `/antigravity:rescue`로 직접 호출 → 무조건 Antigravity 위임
사용자가 `--no-antigravity` 같은 플래그 명시 → 무조건 Claude 본체
...
| > 50,000 토큰 | Antigravity 위임 (1M 컨텍스트 활용) |
...
### 4. Fallback 경로 (실패 시)
- Antigravity 호출 실패 (E_ANTIGRAVITY_NOT_INSTALLED, E_OAUTH_EXPIRED, E_ANTIGRAVITY_FAILED) → Claude 본체로 자동 복귀
...
def route(task: str, user_flags: dict) -> Decision:
    # 1. 사용자 명시 (최우선)
    if user_flags.get("force_antigravity"): return Decision("antigravity", reason="user_explicit")
    if user_flags.get("no_antigravity"):    return Decision("claude", reason="user_explicit")

    # 2. 입력 크기
    estimated_tokens = estimate_tokens(task)
    if estimated_tokens < 5000:    return Decision("claude", reason="too_small")
    if estimated_tokens > 50000:   return Decision("antigravity", reason="too_large")

    # 3. 키워드 매칭
    if matches_antigravity_keywords(task):  return Decision("antigravity", reason="keyword_antigravity")
...
def execute_with_fallback(decision: Decision, task: str):
    ...
    # Antigravity 위임 시도
    try:
        result = antigravity_handle(task)
        if result.error:  raise AntigravityError(result.error)
        return result
    except (AntigravityNotInstalled, OAuthExpired, AntigravityFailed) as e:
...
```jsonl
{"ts":"2026-04-22T...","task":"...","decision":"antigravity","reason":"too_large","fallback_to":"claude","fallback_reason":"E_OAUTH_EXPIRED","extra_cost_estimate":1234}
```
...
4. Fallback은 항상 동작 (Antigravity 실패가 사용자에게 노출되지 않음)
