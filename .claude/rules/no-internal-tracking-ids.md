...
> **전제 갱신 (2026-09-05):** dev/공개 레포 단일화에 따라, §3.2 제외 대상 중 git 이 추적하는 파일 (`.claude/**` 등) 은 공개 레포에도 포함된다. §3.2 의 의미는 "비공개 보장"이 아니라 **"§1 패턴 작성 허용"** 이다. `_workspace/**`·`CLAUDE.local.md`·`보고서/`·`연구노트/` 는 `.gitignore` 미추적으로 계속 미공개다.
...
| 2026-09-05 | §3.2 에 전제 갱신 주석 추가 — 레포 단일화 이후 §3.2 제외 대상 중 git 추적 파일은 공개 레포에 포함됨. 제외의 의미를 "§1 패턴 작성 허용"으로 확정 | 사용자 결정 (2026-09-05): dev 레포와 공개 레포를 이력 포함 단일 레포로 통합 (A안). git 추적된 dev 자산 (`.claude/**`·`CLAUDE.md` 및 과거 이력) 의 추적 ID 노출을 감수. §4.3 의 보류 사유 기록에 해당. §3.1 공개 표면의 작성 금지는 변동 없이 유지. |
