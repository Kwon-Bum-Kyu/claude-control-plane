...
> Claude 를 메인 컨트롤 플레인으로 두고 Antigravity CLI 와 Codex CLI 를 서브에이전트로 오케스트레이션하는 Claude Code 플러그인.
...
대형 컨텍스트(코드베이스, 로그, 문서) 를 Claude 본체로 처리하면 메인 세션 토큰이 급증해 쿼터를 빠르게 소진합니다. CCP 는 이런 작업을 **Antigravity CLI 에 위임**하고, 결과는 3줄 요약 + 디스크 경로로만 메인에 반환하여 **Claude 토큰 누계를 격리**합니다.

### 무엇을 하는가

- **자동 라우팅**: 입력 크기·키워드·사용자 의도·fallback 4축으로 Claude 본체 vs Antigravity 위임을 자동 결정.
- **격리 envelope**: Antigravity 응답 원문은 디스크에만 저장, 메인 세션엔 요약(≤500자) + `result_path` 만 전달.
...
- Ralph 루프 자동화
- ML 기반 분류기 (현재는 규칙 기반)
- Antigravity Vision / Multi-modal 입력
...
- Claude Code v2.1+ 설치
- Node.js ≥ 20.0
- **Antigravity 사용**: Antigravity CLI ≥ 1.0.0 + Google 계정 (자동 안내됨)
...
agy                              # 첫 인터랙티브 실행 시 브라우저로 Google OAuth 자동 시작
# (대체) export ANTIGRAVITY_API_KEY="..."   # AI Studio 발급 키: https://aistudio.google.com/apikey
...
| `--max-tokens N` | ✅ (default 4000) | ❌ | antigravity 응답 토큰 상한 (prompt suffix 변환) |
...
| `--files <glob>` | (대화 첨부) | ⚠️ MVP 미구현 | ❌ | antigravity 백로그 |
...
- antigravity ❌ 표시 옵션이 인자로 새어 들어오면 즉시 `CCP-INVALID-001` (companion 인라인 거부)

---

## 5. 라우터 동작 (3-way)

CCP 라우터는 **4축 우선순위** 로 Claude / Antigravity / Codex 3 경로 중 하나를 결정합니다.

```
사용자 명시 (axis A) → 입력 크기 (axis B) → 키워드 (axis C) → fallback (axis D)
   /antigravity, /codex,        >30K → Antigravity   요약→antigravity / 리뷰→codex    Claude (보수)
...
[사용자 프롬프트]
   ...
   [axis C] 메인 컨텍스트 의존 키워드 (방금/위에서/...) → Claude 강제
            그 외 키워드 사전: codex(코드 리뷰/diff) > antigravity(요약/대용량) > claude
       ↓ (매칭 없으면)
   [axis D] fallback → Claude (보수적)
```

- **자동화 검증**: 70 케이스 회귀 데이터셋에서 정확도 **100%**, P/R ≥ 0.93 모든 모델.
- **투명성**: 모든 호출 결과 `details.mode` 필드에 결정 결과 노출 (`agy` | `codex`).
...
| `CCP-INVALID-001` | ★ | antigravity 측에서 codex 전용 옵션(`--effort`/`--sandbox`/`--write`) 사용 시 — 슬래시 변경 |
...
- **Antigravity 무료 티어 한도는?** 인증 방식에 따라 두 체계가 다릅니다.
...
- **OAuth 만료 주기는?** Codex 는 활성 세션 중 자동 refresh 되며, idle 약 8일 후 stale → 재로그인 필요. Antigravity 의 Google OAuth 는 별도 만료 정책 (정책 변경 가능, 로컬에서 확인 권장). 만료 시 각각 `CCP-OAUTH-001`/`CCP-OAUTH-101` 가 자동 안내합니다.
- **권한 오류 (`npm i -g`)?** nvm 사용 또는 `sudo` 실행. nvm 권장.
- **브라우저 미접근 환경?** Antigravity: `ANTIGRAVITY_API_KEY` 설정 (https://aistudio.google.com/apikey). Codex: `codex login --device-auth` (장치 코드 흐름) 또는 `printenv OPENAI_API_KEY | codex login --with-api-key`.
- **`--effort` 가 antigravity 측에서 거부된다?** 의도된 동작 — 호환성 매트릭스(§4.5) 참고. codex 전용 옵션이므로 `/ccp:codex-rescue --effort high -- "<task>"` 형태로 사용하세요.
...
| 패키지 | 라이선스 | 번들 |
|--------|----------|:----:|
| `Antigravity CLI` (≥ 1.0.0) | proprietary | 외부 (사용자 설치) |
| `@openai/codex` (≥0.122.0) | Apache-2.0 | 외부 (사용자 설치) |
| Node.js (≥20.0) | MIT | 외부 |

번들 바이너리 없음. 외부 API 약관 (Google Antigravity, OpenAI Codex, Anthropic Claude) 은 각 사용자 책임.
...
- 역할 기반 모델 할당 스키마 (사용자가 도메인별로 codex / antigravity / claude 선택)
